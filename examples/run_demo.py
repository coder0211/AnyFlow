...
SCRIPT: dict[str, list[tuple[str, str]]] = {
    ...
    "deploy_prod": [("completed", "Prod deploy #4490, 10% canary. Rollback ready.")],
    "monitor": [("completed", "15 min: error rate back to 0.02% baseline, latency flat. Stable.")],
    "rollback": [("completed", "Rolled back to #4489; metrics recovered.")],
}
...
async def main() -> None:
    ...
    while current is not None:
        ...
        advanced = await call(
            "complete_step",
            {"session_id": session_id, "step_id": step_id, "status": status, "summary": note},
        )
